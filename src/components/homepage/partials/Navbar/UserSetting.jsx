    ...
    <>
      ...
      <li className="flex items-center">
        <button onClick={handleToggleUserSetting}>
          <img src={urlProfileImage} alt="" className="h-[42px] w-[42px] rounded-full border border-slate-300" />
        </button>
      </li>
      {/* Menu Toggle */}
      <div className="absolute top-[50px]">
        {toggleUserSetting && <ChildUserSetting username={username} profileImg={urlProfileImage} handleLogout={handleLogout} />}
      </div>
    </>
  );
};
    ...
    return (
      <li className="w-[95%] py-1 mx-auto hover:bg-slate-100 hover:rounded-lg">
        {url ? (
          ...
          <button onClick={onClick} className="w-[92%] mx-auto flex justify-between items-center">
            <div className="flex gap-3 items-center">
              <div className="bg-slate-300 h-[35px] w-[35px] flex items-center justify-center rounded-full">
                {Icon ? <Icon className="w-[22px] h-[22px]" /> : <MdOutlineDisabledByDefault className="w-[22px] h-[22px]" />}
              </div>
  ...
  return (
    <>
      <div className="bg-white w-[370px] h-[410px] rounded-lg shadow-xl">
        ...
        <div className="bg-white w-[90%] mx-auto relative top-3 flex flex-col justify-start items-start rounded-lg shadow-xl">
          <div className="w-[95%] mx-auto pt-2 border-b-[1px] border-slate-400 pb-3 hover:bg-slate-100 hover:rounded-lg">
            <Link to={`/profile/${authUser.id}`} className="flex gap-3 items-center px-2">
              <img src={profileImg} alt="profile" className="rounded-full w-[42px] h-[42px]" />
        ...
        <div className="mt-7">
          <ul>
            <ListMenuUserSetting title={"Setelan & privasi"} Icon={FaGear} Arrow={RiArrowRightSLine} url={"#"} />
            <ListMenuUserSetting title={"Bantuan & Dukungan"} Icon={AiFillQuestionCircle} Arrow={RiArrowRightSLine} url={"#"} />
            <ListMenuUserSetting title={"Tampilan & Aksesibilitas"} Icon={FaMoon} Arrow={RiArrowRightSLine} url={"#"} />
            <ListMenuUserSetting title={"Beri Masukan"} Icon={RiChatDownloadFill} url={"#"} />

            <ListMenuUserSetting title={"Keluar"} Icon={GrLogout} onClick={handleLogout} />
          </ul>
        </div>
