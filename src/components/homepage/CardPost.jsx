        ...
        <p className="break-words">
          ...
          <span>
            <button
              ...
              className="font-semibold text-sm hover:border-b-[1px] hover:border-black"
            >
              Lihat Selengkapnya ...
            </button>
          </span>
...
export default function CardPost({ userId, posts, refetchAllPosts }) {
  ...
  const LikeButton = ({ postId, userId, initialLiked, refetchAllPosts }) => {
    ...
    return (
      <button
        onClick={handleLike}
        className={`h-[85%] w-1/3 font-semibold text-${hasLike ? 'blue' : 'slate'}-500 flex justify-center items-center gap-2 rounded-sm hover:bg-slate-200`}
      >
          ...
          >
            <div className="bg-white rounded-lg">
                ...
                {postImage && (
                  ...
                  >
                    <img
                      src={`${postImage}`}
                      alt=""
                      className="py-3 max-h-100 max-w-100"
                    />
                  </a>
                )}
              </div>
              ...
              <div className="flex justify-between items-center px-4 pb-1 mt-2">
                ...
                <a
                  href="#"
                  className="hover:border-b-[1px] hover:border-slate-400"
                >
              ...
              <div className={`h-[40px] w-[96%] mx-auto my-2 border-t-[1px] border-slate-400 flex justify-around items-center ${comments.length != 0 || showInputComment === post.id ? 'border-b-[1px] mb-3' : null}`}>
                {/* Like Button */}
                <LikeButton
                  postId={post.id}
                  userId={authUser.id}
