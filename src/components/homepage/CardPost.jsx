  ...
  return (
    ...
    >
      {Icon ? (
        ...
        <MdOutlineDisabledByDefault className="h-[25px] w-[25px]" />
      )}
...
export default function CardPost({ userId, posts, refetchAllPosts }) {
  ...
  const LikeButton = ({ postId, userId, initialLiked, refetchAllPosts }) => {
    ...
    return (
      ...
      >
        <BiLike
          className={`h-[25px] w-[25px] text-${hasLike ? "blue" : "slate"}-500`}
        />
  ...
  if (!posts || posts.length === 0) {
    return (
      <p className=" mt-7 text-center text-slate-400 font-semibold">
        No Post Found !
      </p>
    );
  }
  ...
  return (
    <div>
      {posts.map((post, index) => {
        ...
        const postImage = hasPostImage
          ? `${postImageURL}/${hasPostImage}`
          : null;
        ...
        return (
          <div key={post.id} className="max-h-100 mb-5">
            <div className="bg-white rounded-lg">
              ...
              >
                ...
                <ButtonActionPost
                  ...
                  onClick={() =>
                    setShowInputComment(
                      showInputComment === post.id ? null : post.id
                    )
                  }
                />
            ...
            {lastPost ? (
              <p className="text-center text-slate-400 mt-5 pb-5 font-semibold">
